    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      <div className="bg-[#0c1220] border border-slate-800 p-12 rounded-[4rem] shadow-3xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h2 className="text-5xl font-black text-white tracking-tighter mb-4">Geopolitical Impact Analysis</h2>
          <p className="text-slate-400 font-black uppercase tracking-[0.4em] text-[10px]">High-Ranking Products Affected by Global Atmosphere</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {geopoliticalData.map((item) => (
          <div key={item.id} className="bg-[#0c1220] border border-slate-800 rounded-[3rem] p-10 flex flex-col relative group hover:border-blue-500/50 transition-all shadow-2xl">
            <div className="flex justify-between items-start mb-8">
              <div>
                ...
                <h4 className="text-3xl font-black text-white group-hover:text-blue-400 tracking-tighter leading-none mb-2 truncate mt-3">{item.productName}</h4>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{item.region}</p>
              </div>
            </div>
            <div className="space-y-4 mb-10 flex-1">
              <div className="bg-slate-950/50 p-6 rounded-[2rem] border border-slate-800/50">
                ...
                <p className="text-sm font-bold text-slate-300 leading-relaxed">{item.reason}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800/50">
                  <p className="text-[9px] font-black text-slate-600 uppercase mb-1">Price Change</p>
                  <p className={`text-xs font-bold ${item.priceChange.startsWith('+') ? 'text-emerald-500' : 'text-red-500'}`}>{item.priceChange}</p>
                </div>
                <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800/50">
                  <p className="text-[9px] font-black text-slate-600 uppercase mb-1">Supply Risk</p>
                  <p className="text-xs font-bold text-white">{item.supplyRisk}</p>
                </div>
              </div>
            </div>
            <div className="flex justify-between items-center pt-6 border-t border-slate-800">
               ...
               <button onClick={() => handleLiveRecon(item.productName)} className="text-[10px] font-black text-blue-500 uppercase tracking-widest hover:text-blue-400 transition-colors flex items-center gap-2">
      ...
      {geopoliticalData.length === 0 && (
        <div className="text-center py-40 bg-slate-900/20 rounded-[4rem] border border-dashed border-slate-800">
          <Globe className={`w-16 h-16 text-slate-800 mx-auto mb-6 ${syncing ? 'animate-spin text-blue-500' : 'animate-pulse'}`} />
